...
class TestPlVariance:
    ...
    def test_numerical_parity(self, sample_data):
        ...
        pd_r = variance(sample_data['pd_close'], length=30, talib=False).iloc[35:].values
        pl_r = sample_data['pl_df'].select(pl_variance('close', length=30)).to_series()[35:].to_numpy()
