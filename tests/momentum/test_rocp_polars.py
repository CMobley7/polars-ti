# -*- coding: utf-8 -*-
"""Tests for rocp - Native Polars pl.Expr API + TA-Lib parity."""
...
def test_returns_expr():
    assert isinstance(rocp("close"), pl.Expr)


def test_column_name(spy):
    result = spy.select(rocp("close"))
    assert "ROCP_10" in result.columns
...
def test_native_matches_talib(spy):
    ...
    native = spy.select(rocp("close", talib=False)).to_series().to_numpy()
    ref = talib.ROCP(spy["close"].to_numpy().astype("float64"), timeperiod=10)
...
def test_talib_path_matches_talib(spy):
    ...
    native = spy.select(rocp("close", talib=True)).to_series().to_numpy()
    ref = talib.ROCP(spy["close"].to_numpy().astype("float64"), timeperiod=10)
    m = ~np.isnan(native) & ~np.isnan(ref)
    assert np.max(np.abs(native[m] - ref[m])) < 1e-8
