...
class TestPlFisher:
    ...
    def test_has_fisher_columns(self, sample_df):
        result = sample_df.select(pl_fisher("high", "low"))
    ...
    def test_offset_parameter(self, sample_df):
        result_no_offset = sample_df.select(pl_fisher("high", "low", offset=0))
        result_with_offset = sample_df.select(pl_fisher("high", "low", offset=5))
        assert result_with_offset["FISHERT_9_1"].null_count() > result_no_offset["FISHERT_9_1"].null_count()

    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(pl_fisher("high", "low")).collect()
        assert "FISHERT_9_1" in result.columns

    def test_custom_length(self, sample_df):
        result = sample_df.select(pl_fisher("high", "low", length=14, signal=3))
