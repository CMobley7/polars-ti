...
class TestPlBrar:
    ...
    def test_has_ar_br_columns(self, sample_df):
        result = sample_df.select(pl_brar("open", "high", "low", "close"))
    ...
    def test_offset_parameter(self, sample_df):
        result_no_offset = sample_df.select(pl_brar("open", "high", "low", "close", offset=0))
        result_with_offset = sample_df.select(pl_brar("open", "high", "low", "close", offset=5))
        # Values should be shifted
        assert result_with_offset["AR_26"][:5].null_count() == 5

    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(pl_brar("open", "high", "low", "close")).collect()
        assert "AR_26" in result.columns
        assert "BR_26" in result.columns

    def test_custom_parameters(self, sample_df):
        result = sample_df.select(pl_brar("open", "high", "low", "close", length=10, scalar=50.0, drift=2))
