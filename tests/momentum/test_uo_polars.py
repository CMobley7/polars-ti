...
class TestPlUo:
    ...
    def test_custom_weights(self, sample_data: pl.DataFrame) -> None:
        ...
        result_default = sample_data.select(pl_uo("high", "low", "close", fast_w=4.0, medium_w=2.0, slow_w=1.0, talib=False))
        result_custom = sample_data.select(pl_uo("high", "low", "close", fast_w=1.0, medium_w=1.0, slow_w=1.0, talib=False))
