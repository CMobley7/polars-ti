...
class TestPlCrsi:
    ...
    def test_has_crsi_column(self, sample_df):
        result = sample_df.select(crsi("close"))
    ...
    def test_offset_parameter(self, sample_df):
        result_no_offset = sample_df.select(crsi("close", offset=0, talib=False))
        result_with_offset = sample_df.select(crsi("close", offset=5, talib=False))
        assert result_with_offset["CRSI_3_2_100"].null_count() > result_no_offset["CRSI_3_2_100"].null_count()

    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(crsi("close")).collect()
        assert "CRSI_3_2_100" in result.columns

    def test_talib_true(self, sample_df):
        result = sample_df.select(crsi("close", talib=True))
        assert "CRSI_3_2_100" in result.columns

    def test_talib_false(self, sample_df):
        result = sample_df.select(crsi("close", talib=False))
        assert "CRSI_3_2_100" in result.columns

    def test_custom_parameters(self, sample_df):
        result = sample_df.select(crsi("close", length_rsi=5, length_streak=3, length_rank=50))
