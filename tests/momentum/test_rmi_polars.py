...
class TestPlRmi:
    ...
    def test_offset_shifts_result(self, sample_df):
        ...
        result = sample_df.select(rmi("close", offset=5))
    ...
    def test_with_null_values(self):
        ...
        result = df.select(rmi("close", length=10, momentum=3))
        assert result.height == 50

    def test_lazy_execution(self, sample_df):
        """Test that pl_rmi works with lazy frames."""
        lazy_df = sample_df.lazy()
        result = lazy_df.select(rmi("close")).collect()
