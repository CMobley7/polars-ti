...
class TestPlSlope:
    ...
    def test_output_has_correct_alias_slope(self, sample_data):
        ...
        pl_df, _ = sample_data
        result = pl_df.select(slope("close", length=10))
    ...
    def test_output_has_correct_alias_angle_radians(self, sample_data):
        ...
        pl_df, _ = sample_data
        result = pl_df.select(slope("close", length=10, as_angle=True, to_degrees=False))
    ...
    def test_output_has_correct_alias_angle_degrees(self, sample_data):
        ...
        pl_df, _ = sample_data
        result = pl_df.select(slope("close", length=10, as_angle=True, to_degrees=True))
    ...
    def test_offset_shifts_result(self, sample_data):
        ...
        result_no_offset = pl_df.select(slope("close", offset=0))
        result_with_offset = pl_df.select(slope("close", offset=offset))
    ...
    def test_lazy_execution(self, sample_data):
        ...
        result = lazy_df.select(slope("close", length=10)).collect()
    ...
    def test_different_length_parameters(self, sample_data):
        ...
        for length in [1, 5, 10, 20]:
            result = pl_df.select(slope("close", length=length))
