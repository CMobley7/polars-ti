...
class TestPlCci:
    ...
    def test_returns_expr(self):
        expr = cci("high", "low", "close")
        assert isinstance(expr, pl.Expr)

    def test_has_cci_column(self, sample_df):
        result = sample_df.select(cci("high", "low", "close"))
    ...
    def test_offset_parameter(self, sample_df):
        result = sample_df.select(cci("high", "low", "close", offset=5))
        assert result["CCI_14_0.015"][:5].null_count() == 5

    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(cci("high", "low", "close")).collect()
        assert "CCI_14_0.015" in result.columns

    def test_talib_true(self, sample_df):
        result = sample_df.select(cci("high", "low", "close", talib=True))
        assert "CCI_14_0.015" in result.columns

    def test_talib_false(self, sample_df):
        result = sample_df.select(cci("high", "low", "close", talib=False))
        assert "CCI_14_0.015" in result.columns

    def test_custom_length(self, sample_df):
        result = sample_df.select(cci("high", "low", "close", length=20))
        assert "CCI_20_0.015" in result.columns

    def test_custom_c_constant(self, sample_df):
        result = sample_df.select(cci("high", "low", "close", c=0.02))
    ...
    def test_with_null_values(self):
        ...
        result = df.select(cci("high", "low", "close", talib=False))
