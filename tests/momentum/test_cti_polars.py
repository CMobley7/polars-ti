...
class TestPlCti:
    ...
    def test_has_cti_column(self, sample_df):
        result = sample_df.select(cti("close"))
    ...
    def test_offset_parameter(self, sample_df):
        result_no_offset = sample_df.select(cti("close", offset=0))
        result_with_offset = sample_df.select(cti("close", offset=5))
    ...
    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(cti("close")).collect()
        assert "CTI_12" in result.columns

    def test_custom_length(self, sample_df):
        result = sample_df.select(cti("close", length=20))
