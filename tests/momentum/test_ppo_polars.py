...
class TestPlPpo:
    ...
    def test_talib_parameter_toggle(self, sample_df):
        ...
        result_talib = sample_df.select(ppo("close", talib=True))
        result_native = sample_df.select(ppo("close", talib=False))
    ...
    def test_with_null_values(self):
        ...
        result = df.select(ppo("close"))
        assert result.height == 50

    def test_with_zero_values(self):
        """Test handling of zero prices."""
        df = pl.DataFrame({"close": [0.0] * 20 + [100.0] * 30})
        result = df.select(ppo("close", fast=5, slow=10, signal=3))
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(ppo("close")).collect()
        assert result.height == 100

    def test_different_ma_modes(self, sample_df):
        """Test different MA modes."""
        for mamode in ["sma", "ema"]:
            result = sample_df.select(ppo("close", mamode=mamode))
            unnested = result.unnest(result.columns[0])
