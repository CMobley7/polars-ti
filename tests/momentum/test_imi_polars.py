...
class TestPlImi:
    ...
    def test_has_imi_column(self, sample_df):
        result = sample_df.select(imi("open", "close"))
    ...
    def test_offset_parameter(self, sample_df):
        result_no_offset = sample_df.select(imi("open", "close", offset=0))
        result_with_offset = sample_df.select(imi("open", "close", offset=5))
        assert result_with_offset["IMI_14"].null_count() > result_no_offset["IMI_14"].null_count()

    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(imi("open", "close")).collect()
        assert "IMI_14" in result.columns

    def test_custom_length(self, sample_df):
        result = sample_df.select(imi("open", "close", length=20))
