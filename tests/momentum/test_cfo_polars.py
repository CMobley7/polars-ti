...
class TestPlCfo:
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(cfo("close", length=9)).collect()
        assert "CFO_9" in result.columns
        assert result.height == sample_df.height

    def test_offset_parameter(self, sample_df):
        """Offset parameter shifts results."""
        result_no_offset = sample_df.select(cfo("close", length=9, offset=0))
        result_offset = sample_df.select(cfo("close", length=9, offset=2))
    ...
    def test_scalar_parameter(self, sample_df):
        ...
        result_100 = sample_df.select(cfo("close", length=9, scalar=100.0))
        result_1 = sample_df.select(cfo("close", length=9, scalar=1.0))
