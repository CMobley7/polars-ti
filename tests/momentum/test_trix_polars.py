...
class TestPlTrix:
    ...
    def test_has_trix_column(self, sample_df):
        result = sample_df.select(pl_trix("close"))
    ...
    def test_offset_parameter(self, sample_df):
        result = sample_df.select(pl_trix("close", offset=5))
        trix = result["TRIX"]
        main = trix.struct.field("TRIX_30_9")
        assert main[:5].null_count() == 5

    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(pl_trix("close")).collect()
        assert "TRIX" in result.columns

    def test_talib_true(self, sample_df):
        result = sample_df.select(pl_trix("close", talib=True))
        assert "TRIX" in result.columns

    def test_talib_false(self, sample_df):
        result = sample_df.select(pl_trix("close", talib=False))
        assert "TRIX" in result.columns

    def test_custom_length_parameter(self, sample_df):
        result = sample_df.select(pl_trix("close", length=20, signal=5))
        trix = result["TRIX"]
        assert "TRIX_20_5" in trix.struct.fields

    def test_length_signal_swap(self, sample_df):
        """If length < signal, they should be swapped."""
        result = sample_df.select(pl_trix("close", length=5, signal=20))
        trix = result["TRIX"]
        assert "TRIX_20_5" in trix.struct.fields
