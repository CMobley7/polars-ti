...
class TestPlEri:
    ...
    def test_offset_parameter(self, sample_df):
        result_no_offset = sample_df.select(eri("high", "low", "close", offset=0))
        result_with_offset = sample_df.select(eri("high", "low", "close", offset=5))
        assert result_with_offset["BULLP_13"].null_count() > result_no_offset["BULLP_13"].null_count()

    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(eri("high", "low", "close")).collect()
        assert "BULLP_13" in result.columns

    def test_custom_length(self, sample_df):
        result = sample_df.select(eri("high", "low", "close", length=20))
