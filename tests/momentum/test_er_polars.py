...
class TestPlEr:
    ...
    def test_has_er_column(self, sample_df):
        result = sample_df.select(pl_er("close"))
    ...
    def test_offset_parameter(self, sample_df):
        result_no_offset = sample_df.select(pl_er("close", offset=0))
        result_with_offset = sample_df.select(pl_er("close", offset=5))
        assert result_with_offset["ER_10"].null_count() > result_no_offset["ER_10"].null_count()

    def test_lazy_execution(self, sample_df):
        result = sample_df.lazy().select(pl_er("close")).collect()
        assert "ER_10" in result.columns

    def test_custom_length(self, sample_df):
        result = sample_df.select(pl_er("close", length=20))
