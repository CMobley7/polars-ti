...
class TestPlRoc:
    ...
    def test_talib_parameter(self, sample_df):
        ...
        result = sample_df.select(roc("close", length=10, talib=True))
        arr = result[result.columns[0]].to_numpy()
        valid = ~np.isnan(arr)
        assert valid.sum() > 50
    ...
    def test_with_null_values(self):
        ...
        result = df.select(roc("close", length=5))
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(roc("close", length=10)).collect()
    ...
    def test_roc_calculation_correct(self):
        ...
        result = df.select(roc("close", length=1, scalar=100, talib=False))
        # (110-100)/100 * 100 = 10%
        assert abs(result["ROC_1"][1] - 10.0) < 1e-10

    def test_custom_scalar(self):
        df = pl.DataFrame({"close": [100.0, 110.0]})
        result = df.select(roc("close", length=1, scalar=1, talib=False))
        # (110-100)/100 * 1 = 0.1
