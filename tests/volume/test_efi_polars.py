...
class TestPlEfi:
    ...
    def test_returns_expression(self, sample_df):
        result = sample_df.select(efi("close", "volume"))
        assert result.height == 100

    def test_output_has_correct_alias(self, sample_df):
        result = sample_df.select(efi("close", "volume", length=10))
        assert "EFI_10" in result.columns

    def test_offset_shifts_result(self, sample_df):
        result = sample_df.select(efi("close", "volume", offset=5))
    ...
    def test_with_null_values(self):
        ...
        result = df.select(efi("close", "volume", length=10))
        assert result.height == 50

    def test_lazy_execution(self, sample_df):
        lazy_df = sample_df.lazy()
        result = lazy_df.select(efi("close", "volume")).collect()
        assert "EFI_13" in result.columns

    def test_mamode_parameter(self, sample_df):
        """Different mamode produces valid results."""
        result = sample_df.select(efi("close", "volume", mamode="sma"))
        arr = result[result.columns[0]].to_numpy()
        valid = ~np.isnan(arr)
        assert valid.sum() > 50
