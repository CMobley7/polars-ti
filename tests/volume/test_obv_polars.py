...
class TestPlObv:
    ...
    def test_returns_expression(self, sample_df):
        result = sample_df.select(obv("close", "volume"))
    ...
    def test_offset_shifts_result(self, sample_df):
        result = sample_df.select(obv("close", "volume", offset=5))
    ...
    def test_talib_parameter(self, sample_df):
        ...
        result = sample_df.select(obv("close", "volume", talib=True))
        arr = result[result.columns[0]].to_numpy()
        valid = ~np.isnan(arr)
        assert valid.sum() > 50
    ...
    def test_with_null_values(self):
        ...
        result = df.select(obv("close", "volume"))
        assert result.height == 50

    def test_lazy_execution(self, sample_df):
        lazy_df = sample_df.lazy()
        result = lazy_df.select(obv("close", "volume")).collect()
        assert "OBV" in result.columns
