...
class TestPlPvol:
    ...
    def test_returns_expression(self, sample_df):
        result = sample_df.select(pvol("close", "volume"))
    ...
    def test_signed_parameter(self, sample_df):
        result_unsigned = sample_df.select(pvol("close", "volume", signed=False))
        result_signed = sample_df.select(pvol("close", "volume", signed=True))
    ...
    def test_offset_shifts_result(self, sample_df):
        result = sample_df.select(pvol("close", "volume", offset=5))
        arr = result["PVOL"].to_numpy()
        assert all(np.isnan(arr[:5]))

    def test_with_null_values(self):
        df = pl.DataFrame({"close": [None] + [100.0] * 49, "volume": [None] + [1000.0] * 49})
        result = df.select(pvol("close", "volume"))
        assert result.height == 50

    def test_lazy_execution(self, sample_df):
        lazy_df = sample_df.lazy()
        result = lazy_df.select(pvol("close", "volume")).collect()
        assert "PVOL" in result.columns
