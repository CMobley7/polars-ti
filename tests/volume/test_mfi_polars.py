...
class TestPlMfi:
    ...
    def test_returns_expression(self, sample_df):
        result = sample_df.select(pl_mfi("high", "low", "close", "volume"))
        assert result.height == 100

    def test_output_has_correct_alias(self, sample_df):
        result = sample_df.select(pl_mfi("high", "low", "close", "volume", length=10))
        assert "MFI_10" in result.columns

    def test_talib_parameter(self, sample_df):
        """TA-Lib path produces valid results."""
        result = sample_df.select(pl_mfi("high", "low", "close", "volume", talib=True))
        arr = result[result.columns[0]].to_numpy()
        valid = ~np.isnan(arr)
        assert valid.sum() > 50

    def test_offset_shifts_result(self, sample_df):
        result = sample_df.select(pl_mfi("high", "low", "close", "volume", offset=5))
        arr = result[result.columns[0]].to_numpy()
    ...
    def test_with_null_values(self):
        ...
        result = df.select(pl_mfi("high", "low", "close", "volume", length=10))
        assert result.height == 50

    def test_lazy_execution(self, sample_df):
        lazy_df = sample_df.lazy()
        result = lazy_df.select(pl_mfi("high", "low", "close", "volume")).collect()
