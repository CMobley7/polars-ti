...
class TestPlVfi:
    ...
    def test_returns_expression(self, sample_df):
        result = sample_df.select(pl_vfi("close", "volume"))
    ...
    def test_offset_shifts_result(self, sample_df):
        result = sample_df.select(pl_vfi("close", "volume", offset=5))
        arr = result["VFI_130"].to_numpy()
        assert all(np.isnan(arr[:5]))

    def test_with_null_values(self):
        df = pl.DataFrame({
            "close": [None] + [100.0] * 199,
            "volume": [None] + [1000.0] * 199
        })
        result = df.select(pl_vfi("close", "volume"))
        assert result.height == 200

    def test_lazy_execution(self, sample_df):
        lazy_df = sample_df.lazy()
        result = lazy_df.select(pl_vfi("close", "volume")).collect()
        assert "VFI_130" in result.columns

    def test_custom_length(self, sample_df):
        result = sample_df.select(pl_vfi("close", "volume", length=50))
