...
class TestPlPvr:
    ...
    def test_returns_expression(self, sample_df):
        result = sample_df.select(pl_pvr("close", "volume"))
    ...
    def test_with_null_values(self):
        ...
        result = df.select(pl_pvr("close", "volume"))
        assert result.height == 50

    def test_lazy_execution(self, sample_df):
        lazy_df = sample_df.lazy()
        result = lazy_df.select(pl_pvr("close", "volume")).collect()
        assert "PVR" in result.columns

    def test_drift_parameter(self, sample_df):
        result = sample_df.select(pl_pvr("close", "volume", drift=2))
        arr = result["PVR"].to_numpy()
