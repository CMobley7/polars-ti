...
class TestPlAvwap:
    ...
    def test_returns_list_of_expressions(self, sample_df):
        exprs = avwap("high", "low", "close", "volume")
    ...
    def test_offset_shifts_result(self, sample_df):
        exprs = avwap("high", "low", "close", "volume", offset=5)
        result = sample_df.select(exprs)
    ...
    def test_with_null_values(self):
        ...
        exprs = avwap("high", "low", "close", "volume", left_strength=2, right_strength=2)
    ...
    def test_custom_parameters(self, sample_df):
        exprs = avwap("high", "low", "close", "volume", left_strength=3, right_strength=3)
        result = sample_df.select(exprs)
