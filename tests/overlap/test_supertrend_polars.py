...
class TestPlSupertrend:
    ...
    def test_returns_expr(self):
        expr = supertrend("high", "low", "close")
        assert isinstance(expr, pl.Expr)

    def test_returns_struct(self, sample_df):
        result = sample_df.select(supertrend("high", "low", "close"))
    ...
    def test_custom_parameters(self, sample_df):
        result = sample_df.select(supertrend("high", "low", "close", length=10, multiplier=2.0))
        result = result.unnest(result.columns[0])
    ...
    def test_with_null_values(self):
        ...
        result = df.select(supertrend("high", "low", "close"))
