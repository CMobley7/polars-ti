...
class TestPlMcgd:
    ...
    def test_returns_correct_column(self, sample_df):
        result = sample_df.select(pl_mcgd("close"))
        assert "MCGD_10" in result.columns

    def test_custom_length(self, sample_df):
        result = sample_df.select(pl_mcgd("close", length=14))
    ...
    def test_numerical_parity(self, sample_data):
        ...
        pd_result = mcgd(sample_data['pd_close'], length=10)
        pl_result = sample_data['pl_df'].select(pl_mcgd('close')).to_series()
    ...
    def test_with_null_values(self):
        ...
        result = df.select(pl_mcgd("close"))
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(pl_mcgd("close")).collect()
        assert "MCGD_10" in result.columns
