...
class TestPlMmar:
    ...
    def test_returns_dataframe(self, sample_df):
        result = sample_df.select(mmar("close"))
    ...
    def test_custom_parameters(self, sample_df):
        result = sample_df.select(mmar("close", length=5, step=3, num_ribbons=4))
        result = result.unnest(result.columns[0])
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(mmar("close")).collect()
