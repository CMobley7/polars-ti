...
class TestPlKama:
    ...
    def test_returns_correct_column(self, sample_df):
        result = sample_df.select(kama("close", length=10))
        assert "KAMA_10_2_30" in result.columns

    def test_pure_version(self, sample_df):
        result = sample_df.select(kama("close", length=10, talib=False))
        assert "KAMA_10_2_30" in result.columns
    ...
    def test_talib_version(self, sample_df):
        result = sample_df.select(kama("close", length=10, talib=True))
        assert "KAMA_10_2_30" in result.columns
        arr = result["KAMA_10_2_30"].to_numpy()
        mask = ~np.isnan(arr)
        assert mask.sum() > 50
    ...
    def test_custom_periods(self, sample_df):
        result = sample_df.select(kama("close", length=5, fast=3, slow=20))
        assert "KAMA_5_3_20" in result.columns

    def test_offset(self, sample_df):
        result = sample_df.select(kama("close", offset=5, talib=False))
        arr = result["KAMA_10_2_30"].to_numpy()
    ...
    def test_with_null_values(self):
        ...
        result = df.select(kama("close", talib=False))
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(kama("close", talib=False)).collect()
        assert "KAMA_10_2_30" in result.columns
