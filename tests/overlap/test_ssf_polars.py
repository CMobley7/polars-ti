...
class TestPlSsf:
    ...
    def test_numerical_parity(self, sample_data):
        ...
        pd_result = ssf(sample_data['pd_series'], length=20)
        pl_result = sample_data['pl_df'].select(pl_ssf('close', length=20)).to_series()
    ...
    def test_everget_mode(self, sample_data):
        """Numerical parity for everget mode."""
        pytest.skip("Pandas implementation removed in Phase 4 purge")
        pd_result = ssf(sample_data['pd_series'], length=20, everget=True)
        pl_result = sample_data['pl_df'].select(pl_ssf('close', length=20, everget=True)).to_series()
