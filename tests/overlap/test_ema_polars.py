...
class TestPlEma:
    ...
    def test_numerical_parity_length_10(self, sample_data):
        ...
        pd_result = ema(sample_data['pd_series'], length=10)
        pl_result = sample_data['pl_df'].select(pl_ema('close', length=10)).to_series()
    ...
    def test_numerical_parity_length_20(self, sample_data):
        ...
        pytest.skip("Pandas implementation removed in Phase 4 purge")
        pd_result = ema(sample_data['pd_series'], length=20)
        pl_result = sample_data['pl_df'].select(pl_ema('close', length=20)).to_series()
