...
class TestPlDema:
    ...
    def test_output_has_correct_alias(self, sample_data):
        ...
        result = sample_data["pl_df"].select(dema("close", length=10))
    ...
    def test_offset_shifts_result(self, sample_data):
        ...
        no_offset = sample_data["pl_df"].select(dema("close", length=10)).to_series()
        with_offset = sample_data["pl_df"].select(dema("close", length=10, offset=5)).to_series()
    ...
    def test_warmup_period_has_nan(self, sample_data):
        ...
        result = sample_data["pl_df"].select(dema("close", length=10)).to_series()
    ...
    def test_talib_parameter_accepted(self, sample_data):
        ...
        result = sample_data["pl_df"].select(dema("close", length=10, talib=False))
        assert result is not None

    def test_different_lengths(self, sample_data):
        """Test that different lengths produce different results."""
        result_10 = sample_data["pl_df"].select(dema("close", length=10)).to_series()
        result_20 = sample_data["pl_df"].select(dema("close", length=20)).to_series()
    ...
    def test_with_null_values(self):
        ...
        result = df.select(dema("close", length=10))
    ...
    def test_lazy_execution(self, sample_data):
        ...
        result = lazy_df.select(dema("close", length=10)).collect()
