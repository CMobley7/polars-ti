# -*- coding: utf-8 -*-
"""Unit tests for polars_ti/overlap/swma.py Polars implementation."""
import numpy as np
...
class TestPlSwma:
    """Tests for pl_swma - Symmetric Weighted Moving Average."""
    ...
    def test_returns_expression(self):
        ...
        result = pl_swma("close", length=10)
        assert isinstance(result, pl.Expr)

    def test_output_has_correct_alias(self, sample_data):
        """Output column has correct alias."""
        result = sample_data['pl_df'].select(pl_swma('close', length=10))
        assert result.columns[0] == 'SWMA_10'

    def test_numerical_parity(self, sample_data):
        """Numerical parity with Pandas implementation."""
        pytest.skip("Pandas implementation removed in Phase 4 purge")
        pd_result = swma(sample_data['pd_series'], length=10)
        pl_result = sample_data['pl_df'].select(pl_swma('close', length=10)).to_series()
    ...
    def test_with_null_values(self):
        ...
        result = df.select(pl_swma("close", length=10))
    ...
    def test_lazy_execution(self, sample_data):
        ...
        result = lazy_df.select(pl_swma("close", length=10)).collect()
        assert "SWMA_10" in result.columns
