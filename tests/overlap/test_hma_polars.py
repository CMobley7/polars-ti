...
class TestPlHma:
    ...
    def test_returns_expression(self):
        ...
        result = pl_hma("close", length=10)
        assert isinstance(result, pl.Expr)

    def test_output_has_correct_alias(self, sample_data):
        """Output column has correct alias."""
        result = sample_data['pl_df'].select(pl_hma('close', length=10))
    ...
    def test_numerical_parity(self, sample_data):
        ...
        pd_result = hma(sample_data['pd_series'], length=10)
        pl_result = sample_data['pl_df'].select(pl_hma('close', length=10)).to_series()
    ...
    def test_with_null_values(self):
        ...
        result = df.select(pl_hma("close", length=10))
    ...
    def test_lazy_execution(self, sample_data):
        ...
        result = lazy_df.select(pl_hma("close", length=10)).collect()
