...
class TestPlHwma:
    ...
    def test_returns_correct_column(self, sample_df):
        result = sample_df.select(hwma("close"))
        assert "HWMA_0.2_0.1_0.1" in result.columns

    def test_custom_parameters(self, sample_df):
        result = sample_df.select(hwma("close", na=0.3, nb=0.2, nc=0.15))
        assert "HWMA_0.3_0.2_0.15" in result.columns

    def test_different_params_give_different_results(self, sample_df):
        r1 = sample_df.select(hwma("close", na=0.2)).get_column("HWMA_0.2_0.1_0.1").to_numpy()
        r2 = sample_df.select(hwma("close", na=0.5)).get_column("HWMA_0.5_0.1_0.1").to_numpy()
        assert not np.allclose(r1, r2)

    def test_offset(self, sample_df):
        result = sample_df.select(hwma("close", offset=5))
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(hwma("close")).collect()
        assert "HWMA_0.2_0.1_0.1" in result.columns
