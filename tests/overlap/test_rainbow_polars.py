...
class TestPlRainbow:
    ...
    def test_returns_struct(self, sample_df):
        result = sample_df.select(rainbow("close"))
    ...
    def test_custom_ribbons(self, sample_df):
        result = sample_df.select(rainbow("close", num_ribbons=5))
        unnested = result.unnest(result.columns[0])
    ...
    def test_with_null_values(self):
        ...
        result = df.select(rainbow("close"))
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(rainbow("close")).collect()
