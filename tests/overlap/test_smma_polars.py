...
class TestPlSmma:
    ...
    def test_returns_correct_column(self, sample_df):
        result = sample_df.select(smma("close", length=7))
        assert "SMMA_7" in result.columns

    def test_different_lengths(self, sample_df):
        r7 = sample_df.select(smma("close", length=7))
        r14 = sample_df.select(smma("close", length=14))
    ...
    def test_offset(self, sample_df):
        result = sample_df.select(smma("close", length=7, offset=5))
        arr = result["SMMA_7"].to_numpy()
    ...
    def test_with_null_values(self):
        ...
        result = df.select(smma("close", length=7))
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(smma("close", length=7)).collect()
        assert "SMMA_7" in result.columns
