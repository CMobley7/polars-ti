# -*- coding: utf-8 -*-
"""Tests for tsf - Native Polars pl.Expr API + TA-Lib parity."""
...
def test_returns_expr():
    assert isinstance(tsf("close"), pl.Expr)


def test_column_name(spy):
    result = spy.select(tsf("close"))
    assert "TSF_14" in result.columns
...
def test_native_matches_talib(spy):
    ...
    native = spy.select(tsf("close", talib=False)).to_series().to_numpy()
    ref = talib.TSF(spy["close"].to_numpy().astype("float64"), timeperiod=14)
...
def test_talib_path_matches_talib(spy):
    ...
    native = spy.select(tsf("close", talib=True)).to_series().to_numpy()
    ref = talib.TSF(spy["close"].to_numpy().astype("float64"), timeperiod=14)
    m = ~np.isnan(native) & ~np.isnan(ref)
