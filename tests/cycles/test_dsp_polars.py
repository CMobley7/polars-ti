...
class TestPlDsp:
    ...
    def test_basic_calculation(self):
        ...
        result = df.select(dsp("close", length=14))
    ...
    def test_dsp_oscillates_around_zero(self):
        ...
        result = df.select(dsp("close", length=10))
    ...
    def test_lazy_execution(self):
        ...
        df = pl.DataFrame({"close": close})
        lazy_df = df.lazy()
        result = lazy_df.select(dsp("close", length=14)).collect()
        assert "DSP_14" in result.columns

    def test_offset_parameter(self):
        """Offset parameter shifts results."""
        df = pl.DataFrame({"close": [100.0] * 30})
        result = df.select(dsp("close", length=14, offset=2))
