...
class TestPlAtrts:
    ...
    def test_returns_expression(self, sample_data):
        result = pl_atrts("high", "low", "close")
        assert isinstance(result, pl.Expr)

    def test_output_has_correct_alias(self, sample_data):
        df = pl.DataFrame(sample_data)
        result = df.select(pl_atrts("high", "low", "close", length=14, ma_length=20, multiplier=3.0))
        assert "ATRTS" in result.columns[0]
    ...
    def test_numerical_parity(self, sample_data):
        ...
        pd_result = atrts(pd_high, pd_low, pd_close, length=14, ma_length=20, multiplier=3.0, talib=False)
        pl_result = pl_df.select(pl_atrts("high", "low", "close", length=14, ma_length=20, multiplier=3.0))
    ...
    def test_with_null_values(self, sample_data):
        ...
        result = df.select(pl_atrts("high", "low", "close"))
    ...
    def test_lazy_execution(self, sample_data):
        df = pl.DataFrame(sample_data)
        result = df.lazy().select(pl_atrts("high", "low", "close")).collect()
        assert result.height == 100
