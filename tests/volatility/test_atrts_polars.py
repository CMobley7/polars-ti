...
class TestPlAtrts:
    ...
    def test_returns_expression(self, sample_data):
        result = atrts("high", "low", "close")
        assert isinstance(result, pl.Expr)

    def test_output_has_correct_alias(self, sample_data):
        df = pl.DataFrame(sample_data)
        result = df.select(atrts("high", "low", "close", length=14, ma_length=20, multiplier=3.0))
    ...
    def test_with_null_values(self, sample_data):
        ...
        result = df.select(atrts("high", "low", "close"))
    ...
    def test_lazy_execution(self, sample_data):
        df = pl.DataFrame(sample_data)
        result = df.lazy().select(atrts("high", "low", "close")).collect()
        assert result.height == 100
