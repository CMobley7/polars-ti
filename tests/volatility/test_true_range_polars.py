...
class TestPlTrueRange:
    ...
    def test_returns_expression(self, sample_data):
        result = pl_true_range("high", "low", "close")
        assert isinstance(result, pl.Expr)

    def test_output_has_correct_alias(self, sample_data):
        df = pl.DataFrame(sample_data)
        result = df.select(pl_true_range("high", "low", "close"))
        assert "TRUERANGE" in result.columns[0]
    ...
    def test_numerical_parity(self, sample_data):
        ...
        pd_result = true_range(pd_high, pd_low, pd_close, talib=False, drift=1)
        pl_result = pl_df.select(pl_true_range("high", "low", "close", drift=1))
        warmup = 2
    ...
    def test_lazy_execution(self, sample_data):
        df = pl.DataFrame(sample_data)
        result = df.lazy().select(pl_true_range("high", "low", "close")).collect()
        assert result.height == 100
