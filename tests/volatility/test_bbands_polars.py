# -*- coding: utf-8 -*-
"""Tests for pl_bbands."""
import numpy as np
import pandas as pd  # REMOVED: pandas dependency  # Restored for fixtures
import polars as pl
import pytest
from polars_ti.volatility.bbands import pl_bbands


class TestPlBbands:
    @pytest.fixture
    def sample_data(self):
        np.random.seed(42)
        n = 100
        close = 100 + np.cumsum(np.random.randn(n) * 0.5)
        return {"close": close}

    def test_returns_expression(self, sample_data):
        result = pl_bbands("close")
        assert isinstance(result, pl.Expr)

    def test_output_has_correct_alias(self, sample_data):
        df = pl.DataFrame(sample_data)
        result = df.select(pl_bbands("close", length=5, std=2.0))
        assert "BBANDS" in result.columns[0]

    def test_numerical_parity_polars(self, sample_data):
        pytest.skip("Pandas implementation removed in Phase 4 purge")
        pd_close = pd.Series(sample_data["close"])
        pl_df = pl.DataFrame(sample_data)
        
        pd_result = bbands(pd_close, length=5, std=2.0, ddof=1, talib=False)
        pl_result = pl_df.select(pl_bbands("close", length=5, std=2.0, ddof=1, talib=False))
        pl_unnest = pl_result.unnest(pl_result.columns[0])
        
        warmup = 10
        for col in ["BBL_5_2.0", "BBM_5_2.0", "BBU_5_2.0"]:
            pd_vals = pd_result[col].to_numpy()[warmup:]
            pl_vals = pl_unnest[col].to_numpy()[warmup:]
            mask = np.isfinite(pd_vals) & np.isfinite(pl_vals)
            if mask.sum() > 0:
                max_diff = np.abs(pd_vals[mask] - pl_vals[mask]).max()
                assert max_diff < 1e-6, f"{col} parity failed: {max_diff}"

    def test_talib_option(self, sample_data):
        """Test TA-Lib path if available."""
        pytest.skip("Pandas implementation removed in Phase 4 purge")
        pd_close = pd.Series(sample_data["close"])
        pl_df = pl.DataFrame(sample_data)
        try:
            pd_result = bbands(pd_close, length=5, std=2.0, talib=True)
            pl_result = pl_df.select(pl_bbands("close", length=5, std=2.0, talib=True))
            pl_unnest = pl_result.unnest(pl_result.columns[0])
            warmup = 10
            for col in ["BBL_5_2.0", "BBM_5_2.0", "BBU_5_2.0"]:
                pd_vals = pd_result[col].to_numpy()[warmup:]
                pl_vals = pl_unnest[col].to_numpy()[warmup:]
                mask = np.isfinite(pd_vals) & np.isfinite(pl_vals)
                if mask.sum() > 0:
                    max_diff = np.abs(pd_vals[mask] - pl_vals[mask]).max()
                    assert max_diff < 1e-6, f"{col} talib parity failed: {max_diff}"
        except ImportError:
            pytest.skip("TA-Lib not installed")

    def test_with_null_values(self, sample_data):
        data = sample_data.copy()
        data["close"] = data["close"].copy()
        data["close"][10:15] = np.nan
        df = pl.DataFrame(data)
        result = df.select(pl_bbands("close"))
        assert result.height == 100

    def test_with_zeros(self, sample_data):
        data = sample_data.copy()
        data["close"] = data["close"].copy()
        data["close"][50:55] = 100.0
        df = pl.DataFrame(data)
        result = df.select(pl_bbands("close"))
        assert result.height == 100

    def test_lazy_execution(self, sample_data):
        df = pl.DataFrame(sample_data)
        result = df.lazy().select(pl_bbands("close")).collect()
        assert result.height == 100
