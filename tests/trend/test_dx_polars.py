# -*- coding: utf-8 -*-
"""Tests for dx - Native Polars pl.Expr API + TA-Lib parity."""
...
def test_returns_expr():
    assert isinstance(dx("high", "low", "close"), pl.Expr)


def test_column_name(spy):
    result = spy.select(dx("high", "low", "close"))
    assert "DX_14" in result.columns
...
def test_native_matches_talib(spy):
    ...
    native = spy.select(dx("high", "low", "close", talib=False)).to_series().to_numpy()
    ref = talib.DX(
...
def test_talib_path_matches_talib(spy):
    ...
    native = spy.select(dx("high", "low", "close", talib=True)).to_series().to_numpy()
    ref = talib.DX(
        spy["high"].to_numpy().astype("float64"),
        spy["low"].to_numpy().astype("float64"),
        spy["close"].to_numpy().astype("float64"),
