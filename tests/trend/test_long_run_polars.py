...
class TestPlLongRun:
    ...
    def test_returns_expression(self, sample_df):
        result = sample_df.select(long_run("fast", "slow"))
        assert result.height == 100

    def test_output_has_correct_alias(self, sample_df):
        result = sample_df.select(long_run("fast", "slow", length=3))
        assert "LR_3" in result.columns

    def test_offset_shifts_result(self, sample_df):
        result = sample_df.select(long_run("fast", "slow", offset=5))
    ...
    def test_with_null_values(self):
        ...
        result = df.select(long_run("fast", "slow", length=2))
    ...
    def test_lazy_execution(self, sample_df):
        ...
        result = lazy_df.select(long_run("fast", "slow", length=2)).collect()
