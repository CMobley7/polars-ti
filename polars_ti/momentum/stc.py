...
def nb_schaff_tc(xmacd: np.ndarray, tclength: int, factor: float):
    ...
    lowest_macd = rolling_extreme(xmacd, tclength, False, 1)[0]
    highest_macd = rolling_extreme(xmacd, tclength, True, 1)[0]
    for i in range(1, m):
        lowest_xmacd = lowest_macd[i]
        highest_xmacd = highest_macd[i]
    ...
    lowest_smooth = rolling_extreme(pf, tclength, False, 1)[0]
    highest_smooth = rolling_extreme(pf, tclength, True, 1)[0]
    for i in range(1, m):
        lowest_pf = lowest_smooth[i]
        highest_pf = highest_smooth[i]
