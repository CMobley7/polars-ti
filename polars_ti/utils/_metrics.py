...
def log_return(close: IntoExpr) -> PlExpr:
...
def percent_return(close: IntoExpr) -> PlExpr:
...
def cumulative_return(close: IntoExpr) -> PlExpr:
...
def rolling_volatility(close: IntoExpr, length: int = 20) -> PlExpr:
...
def drawdown(close: IntoExpr) -> PlExpr:
...
def max_drawdown(df: pl.DataFrame, close_col: str) -> float:
